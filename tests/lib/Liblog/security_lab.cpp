...
void stress_worker(int thread_id, const fs::path &secure_path,
                   const fs::path &insecure_path) {
  std::string SEMsg;
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    int rc =
        chk_file_sec(secure_path, false, true, S_IWGRP | S_IWOTH, true, SEMsg);
    if (rc != 0) {
      failure_count++;
    }
    ...
    rc =
        chk_file_sec(insecure_path, true, true, S_IWGRP | S_IWOTH, true, SEMsg);
    if (rc != EACCES) {
      failure_count++;
    }
  }
...
int main() {
  ...
  std::cout << "Launching " << thread_count
            << " threads for security stress test...\n";
