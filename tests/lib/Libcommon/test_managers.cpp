...
protected:
    ...
    void tear_down_files() {
        if (fs::exists("test_nodes.bin")) fs::remove("test_nodes.bin");
        if (fs::exists("test_queues.bin")) fs::remove("test_queues.bin");
        if (fs::exists("test_jobs.bin")) fs::remove("test_jobs.bin");
    }
};
...
TEST_F(ManagerTest, TestNodeStatusUpdate) {
    ...
    // Verify update doesn't throw and potentially check internal state if API allows
...
TEST_F(ManagerTest, TestPersistence) {
    ...
    EXPECT_NO_THROW({
        JobIndexManager job_mgr2("test_jobs.bin", 1000);
    });
}