...
int main() {
  ...
  dispatcher.registerStatusHandler([](const torque_ng::JobStatusRequest &req) {
    ...
    std::vector<DummyJob> jobs = {{"101.local", "Compile_Task", "Running", "0"},
    ...
    for (const auto &j : jobs) {
      ...
      info->set_job_state(
          torque_ng::utils::JobStateMapper::from_string(j.state));
      info->set_job_substate(
          torque_ng::utils::JobStateMapper::sub_from_string(j.substate));
    }
  ...
  std::cout << "Received " << data.jobs_size() << " jobs from dispatcher."
            << std::endl;
