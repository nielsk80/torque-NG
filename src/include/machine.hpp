...
class Core {
  ...
  vector<bool>
      is_index_busy; // Tells whether or not each processing unit is busy
...
public:
  ...
  void initialize_accelerators_from_strings(std::string &gpus,
                                            std::string &mics);
...
#endif
  void parse_values_from_json_string(const Json::Value &layout,
                                     std::string &cores, std::string &threads,
                                     std::string &gpus, std::string &mics,
  ...
  void calculateStepCounts(const int lprocs_per_task, const int pu_per_task,
                           int &step, int &step_rem, int &place_count,
                           int &place_count_rem);
  bool spread_place(req &r, allocation &master, int execution_slots_per,
                    int &remainder);
  bool spread_place_cores(req &r, allocation &master, int &remaining_cores,
                          int &remaining_lprocs, int &gpus, int &mics);
  bool spread_place_threads(req &r, allocation &master, int &remaining_cores,
                            int &remaining_lprocs, int &gpus, int &mics);
  ...
  void place_tasks_execution_slots(int to_bind, int to_place, allocation &a,
                                   int type);
  void place_task_for_legacy_threads(int threads_to_bind, int threads_to_place,
                                     allocation &master, allocation &a);
  ...
  void initialize_allocations(const Json::Value &layout,
                              std::vector<std::string> &valid_ids);
  void initialize_allocation(const Json::Value &layout,
                             std::vector<std::string> &valid_ids);
  ...
  bool getOpenThreadVector(std::vector<int> &slots,
                           int execution_slots_per_task);
  bool getContiguousThreadVector(std::vector<int> &slots,
                                 int execution_slots_per_task);
  bool getContiguousCoreVector(std::vector<int> &slots,
                               int execution_slots_per_task);
...
public:
  ...
  int getid();
  hwloc_uint64_t getMemory() const;
  hwloc_uint64_t get_memory_for_completely_free_chips(unsigned long diff,
                                                      int &count) const;
  ...
  void place_all_execution_slots(req &r, allocation &task_alloc);
  bool spread_place(req &r, allocation &master, int execution_slots_per,
                    int &remainder, bool chips);
  bool spread_place_pu(req &r, allocation &task_alloc, int &cores, int &lprocs,
                       int &gpus, int &mics);
  int place_task(req &r, allocation &a, int to_place, const char *hostname);
  bool free_task(const char *jobid);
...
#ifdef NVIDIA_GPUS
#ifdef NVML_API
  hwloc_obj_t get_non_nvml_device(hwloc_topology_t topology,
                                  nvmlDevice_t device);
#endif
#endif
...
public:
  ...
  void place_all_execution_slots(req &r, allocation &master,
                                 const char *hostname);
  int spread_place(req &r, allocation &master, int tasks_for_node,
                   const char *hostname);
  int spread_place_pu(req &r, allocation &master, int tasks_for_node,
                      const char *hostname);
  int place_job(job *pjob, string &cpu_string, string &mem_string,
                const char *hostname, bool legacy_vmem);
  ...
  int fit_tasks_within_sockets(req &r, allocation &job_alloc,
                               const char *hostname, int &remaining_tasks);
  ...
  bool check_if_possible(int &sockets, int &numa_nodes, int &cores,
                         int &threads) const;
