...
public:
  pmix_operation(job *pjob, const pmix_proc_t procs[], size_t nprocs,
                 const std::string &data, pmix_modex_cbfunc_t cbfunc,
                 void *cbdata); // used for fence
  pmix_operation();
  pmix_operation(job *pjob, const pmix_proc_t procs[], size_t nprocs,
                 pmix_op_cbfunc_t to_call,
                 void *cbdata);          // used for connect
  pmix_operation(char *data, job *pjob); // used for connect
  pmix_operation(job *pjob, const pmix_proc_t procs[], size_t nprocs,
                 pmix_op_cbfunc_t to_call, void *cbdata, int operation_type);
  ...
  int connection_process_terminated(job *pjob, struct sockaddr_in *source_addr,
                                    bool am_i_ms);
};
...
int clean_up_connection(job *pjob, struct sockaddr_in *source_addr,
                        unsigned int op_id, bool ms);
