...
public:
  tcpData() : data() { memset(&mutex, 0, sizeof(mutex)); }
...
int tcp_read(
    ...
    struct tcp_chan *chan, long long *read_len, long long *avail_len)

{
    ...
    *avail_len = tp->tdis_eod - tp->tdis_leadp;
    ...
    if (max_read_len > tdis_buf_len)
      ...
      snprintf(err_msg, sizeof(err_msg),
        "eod ptr BEYOND end of buffer!! (fit) Remaining buffer = %d, read_len =
    %lld", max_read_len, *read_len); log_err(PBSE_INTERNAL,__func__,err_msg);
      }
      *****************/
  ...
  /* data read is greater than buffer size */
    ...
    *avail_len = tp->tdis_eod - tp->tdis_leadp;
    ...
    if (max_read_len > tdis_buf_len)
      {
      snprintf(err_msg, sizeof(err_msg), "eod ptr BEYOND end of buffer!!(expand)
    Remaining buffer = %d, read_len = %lld", max_read_len, *read_len);
...
int tcp_rskip(
    ...
    struct tcp_chan *chan, size_t ct)
...
int tcp_gets(
    ...
    struct tcp_chan *chan, char *str, size_t ct, unsigned int timeout)
...
int tcp_getc(
    ...
    struct tcp_chan *chan, unsigned int timeout)
  ...
  if ((tp->tdis_thebuf + tp->tdis_bufsize - tp->tdis_leadp) < (ssize_t)ct) {
    ...
    if (!temp) {
      ...
      snprintf(log_buf,sizeof(log_buf),
        "out of space in buffer and cannot calloc message buffer (bufsize=%ld,
      buflen=%d, ct=%d)\n", tp->tdis_bufsize, (int)(tp->tdis_leadp -
      tp->tdis_thebuf), (int)ct); log_err(ENOMEM, __func__, log_buf);
      *************************/
      return (-1);
    }
...
int tcp_rcommit(
    ...
    struct tcp_chan *chan, int commit_flag)
