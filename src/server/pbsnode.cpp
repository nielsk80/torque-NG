...
pbsnode::pbsnode()
    : nd_error(0), nd_properties(), nd_version(0),
      nd_plugin_generic_resources(), nd_plugin_generic_metrics(),
      nd_plugin_varattrs(), nd_plugin_features(), nd_proximal_failures(0),
      nd_consecutive_successes(0), nd_mutex(), nd_id(-1), nd_f_st(), nd_addrs(),
      nd_prop(NULL), nd_status(), nd_note(), nd_stream(-1), nd_flag(okay),
      nd_mom_port(PBS_MOM_SERVICE_PORT),
      nd_mom_rm_port(PBS_MANAGER_SERVICE_PORT), nd_sock_addr(), nd_nprops(0),
      nd_nstatus(0), nd_slots(), nd_job_usages(), nd_needed(0),
      nd_np_to_be_used(0), nd_state(INUSE_DOWN), nd_ntype(0), nd_order(0),
      nd_warnbad(0), nd_lastupdate(0), nd_lastHierarchySent(0),
      nd_hierarchy_level(0), nd_in_hierarchy(0), nd_ngpus(0), nd_gpus_real(0),
      nd_gpusn(), nd_ngpus_free(0), nd_ngpus_needed(0), nd_ngpus_to_be_used(0),
      nd_gpustatus(NULL), nd_ngpustatus(0), nd_nmics(0), nd_micstatus(NULL),
      nd_micjobids(), nd_nmics_alloced(0), nd_nmics_free(0),
      nd_nmics_to_be_used(0), parent(NULL), num_node_boards(0),
      node_boards(NULL), numa_str(), gpu_str(), nd_mom_reported_down(0),
      nd_is_alps_reporter(0), nd_is_alps_login(0), nd_ms_jobs(NULL),
      alps_subnodes(NULL), max_subnode_nppn(0), nd_power_state(0),
      nd_power_state_change_time(0), nd_acl(NULL), nd_requestid(),
      nd_tmp_unlock_count(0)
...
#endif
{
  if (hierarchy_handler.isHiearchyLoaded()) {
    this->nd_state |=
        INUSE_NOHIERARCHY; // This is a dynamic add so don't allow
...
pbsnode::pbsnode(
    ...
    const char *pname, u_long *pul, bool skip_address_lookup)
    : nd_error(0), nd_properties(), nd_version(0),
      nd_plugin_generic_resources(), nd_plugin_generic_metrics(),
      nd_plugin_varattrs(), nd_plugin_features(), nd_proximal_failures(0),
      nd_consecutive_successes(0), nd_mutex(), nd_f_st(), nd_prop(NULL),
      nd_status(), nd_note(), nd_stream(-1), nd_flag(okay),
      nd_mom_port(PBS_MOM_SERVICE_PORT),
      nd_mom_rm_port(PBS_MANAGER_SERVICE_PORT), nd_sock_addr(), nd_nprops(0),
      nd_nstatus(0), nd_slots(), nd_job_usages(), nd_needed(0),
      nd_np_to_be_used(0), nd_state(INUSE_DOWN), nd_ntype(0), nd_order(0),
...

{
  ...
  if (!skip_address_lookup) {
    if (pbs_getaddrinfo(pname, NULL, &pAddrInfo))
      ...
      memcpy(&this->nd_sock_addr, pAddrInfo->ai_addr,
...
pbsnode::pbsnode(
    ...
    const pbsnode &other)
    : nd_error(other.nd_error), nd_properties(other.nd_properties),
      ...
      nd_job_usages(other.nd_job_usages), nd_needed(other.nd_needed),
      nd_np_to_be_used(other.nd_np_to_be_used), nd_state(other.nd_state),
      nd_ntype(other.nd_ntype), nd_order(other.nd_order),
      ...
      nd_ngpustatus(other.nd_ngpustatus), nd_nmics(other.nd_nmics),
...
int pbsnode::lock_node(
    ...
    const char *id, const char *msg, int logging)

{
  ...
  if (logging >= 10) {
    if (msg == NULL)
      msg = stub_msg;
    snprintf(err_msg, MSG_LEN_LONG, "locking start %s in method %s-%s",
             this->nd_name.c_str(), id, msg);
    log_record(PBSEVENT_DEBUG, PBS_EVENTCLASS_NODE, __func__, err_msg);
  }

  if (pthread_mutex_lock(&this->nd_mutex) != 0) {
    if (logging >= 10) {
      snprintf(err_msg, MSG_LEN_LONG,
               "ALERT: cannot lock node %s mutex in method %s",
  ...
  if (logging >= 10) {
    snprintf(err_msg, MSG_LEN_LONG, "locking complete %s in method %s",
             this->nd_name.c_str(), id);
    log_record(PBSEVENT_DEBUG, PBS_EVENTCLASS_NODE, __func__, err_msg);
  }
...

{
  ...
  if (logging >= 10) {
    if (msg == NULL)
      msg = stub_msg;
    snprintf(err_msg, MSG_LEN_LONG, "unlocking %s in method %s-%s",
             this->nd_name.c_str(), id, msg);
    log_record(PBSEVENT_DEBUG, PBS_EVENTCLASS_NODE, __func__, err_msg);
  }
  ...
  if (pthread_mutex_unlock(&this->nd_mutex) != 0) {
    if (logging >= 10) {
      snprintf(err_msg, MSG_LEN_LONG,
               "ALERT: cannot unlock node %s mutex in method %s",
               this->nd_name.c_str(), id);
      log_record(PBSEVENT_DEBUG, PBS_EVENTCLASS_NODE, __func__, err_msg);
    }
    rc = PBSE_MUTEX;
 ...
 * They are written in the format <node_name> cray_compute feature1[
 * feature2[...]]
 *
...

{
  ...
  while ((alps_node = next_host(this->alps_subnodes, &iter, NULL)) != NULL) {
    /* only write nodes that have more than just cray_compute as their
     * properties. Checking for > 2 properties should be sufficient -- all
     * computes have cray_compute and all nodes have their name as a property */
...

{
  ...
  if (this->nd_slots.get_total_execution_slots() > 1)
    fprintf(nin, " %s=%d", ATTR_NODE_np,
            this->nd_slots.get_total_execution_slots());
  ...
  if (this->num_node_boards > 0) {
    fprintf(nin, " %s=%d", ATTR_NODE_num_node_boards, this->num_node_boards);
  }
  ...
  if ((this->nd_acl != NULL) && (this->nd_acl->as_usedptr != 0)) {
  ...
  if ((cray_enabled == true) && (this == alps_reporter))
    ...
    if (this->nd_consecutive_successes > 1) {
      ...
      if (this->nd_state & INUSE_NETWORK_FAIL) {
        snprintf(log_buf, sizeof(log_buf),
                 "Node '%s' has had two or more consecutive network successes, "
                 "marking online.",
                 this->nd_name.c_str());
...
int pbsnode::get_version() const { return (this->nd_version); }

/*
 * set_version()
 *
 * Sets this node's version from a string in the format
 * version=\d.\d.\d[.hotfixinformation] The hotfix information, if any, is
 * ignored
 */
    ...
    for (size_t i = 0; i < keys.size(); i++) {
      if (keys[i] == GRES) {
        ...
        for (size_t j = 0; j < resource_keys.size(); j++) {
          this->nd_plugin_generic_resources[resource_keys[j]] =
              gres_info[resource_keys[j]].asInt();
        }
        ...
        for (size_t j = 0; j < varattr_keys.size(); j++) {
          this->nd_plugin_varattrs[varattr_keys[j]] =
              varattr_info[varattr_keys[j]].asString();
        }
        ...
        for (size_t j = 0; j < gmetric_keys.size(); j++)
          this->nd_plugin_generic_metrics[gmetric_keys[j]] =
              gmetric_info[gmetric_keys[j]].asDouble();
...

{
  ...
  for (std::map<std::string, unsigned int>::const_iterator it =
           this->nd_plugin_generic_resources.begin();
       it != this->nd_plugin_generic_resources.end(); it++) {
  ...
  for (std::map<std::string, double>::const_iterator it =
           this->nd_plugin_generic_metrics.begin();
       it != this->nd_plugin_generic_metrics.end(); it++) {
  ...
  for (std::map<std::string, std::string>::const_iterator it =
           this->nd_plugin_varattrs.begin();
       it != this->nd_plugin_varattrs.end(); it++) {
  ...
  if (this->nd_plugin_features.size() > 0) {
    svrattrl *pal =
        attrlist_create(FEATURES, NULL, this->nd_plugin_features.size() + 1);
