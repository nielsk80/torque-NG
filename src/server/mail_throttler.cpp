...
mail_info::mail_info()
    : mailto(), exec_host(), jobid(), jobname(), text(), errFile(), outFile(),
      mail_point(0) {}
...
mail_info::mail_info(
    ...
    : mailto(other.mailto), exec_host(other.exec_host), jobid(other.jobid),
...
int mail_throttler::get_email_list(
    ...
    const std::string &addressee, std::vector<mail_info> &list)
 ...
 * @return true if this is a new recipient or false if this recipient already
 * exists
 */
