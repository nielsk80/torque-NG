...
execution_slot_tracker::execution_slot_tracker()
    : open_count(0) {} /* END default contructor */
...
/*
 * unset_subset()
 * @pre-cond: subset must be of an equal or smaller size than this execution
 * slot tracker object
 * @post-cond: all of the occupied slots in subset will become unoccupied in
 * this
 */
 ...
 * @post-cond: slots[index] will be occupied and open_count will be updated if
 * needed
 ...
 * @pre-cond: index must be a valid index into the vector
 * @post-cond: slots[index] will be free and open_count will be updated if
 * needed
 * @return PBSE_NONE on success or OUT_OF_RANGE if index isn't a valid index
 */
...
int execution_slot_tracker::reserve_execution_slot(
    ...
    int index, execution_slot_tracker &subset)
 ...
 * reserves num_slots_to_reserve from this and marks the same ones as occupied
 * in est
 * @pre-cond: est must be a valid execution_slot_tracker object
 * @post-cond: both this and est will have num_slots_to_reserve more slots set
 * as occupied. est will be resized if necessary to accomodate this
 * functionality.
...
int execution_slot_tracker::reserve_execution_slots(
    ...
    int num_slots_to_reserve, execution_slot_tracker &est)

{
  ...
  for (int i = 0;
       i < this->slots.size() && reserved_so_far < num_slots_to_reserve; i++) {
