...
restricted_host::restricted_host(
    ...
    const std::string &host, const std::string &user, const std::string &group)
    : hostname(host), users(), groups()
...

{
  ...
  for (it = this->groups.begin();
       it != this->groups.end() && authorized == false; it++) {
