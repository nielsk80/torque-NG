...
int create_and_queue_array_subjob(job_array *pa, mutex_mgr &array_mgr,
                                  job *template_job,
                                  mutex_mgr &template_job_mgr, int index,
                                  std::string &prev_job_id, bool place_hold);

const int DEFAULT_IDLE_SLOT_LIMIT = 300;

// array_info empty constructor
array_info::array_info()
    : struct_version(ARRAY_QS_STRUCT_VERSION), array_size(0), num_jobs(0),
      slot_limit(NO_SLOT_LIMIT), jobs_running(0), jobs_done(0), num_cloned(0),
      num_started(0), num_failed(0), num_successful(0), num_purged(0),
      num_idle(0), deps(), idle_slot_limit(DEFAULT_IDLE_SLOT_LIMIT),
      highest_id_created(-1), range_str()
...

{
  ...
  for (std::list<array_depend *>::iterator it = this->deps.begin();
       it != this->deps.end(); it++) {
...
job_array::job_array()
    : job_ids(NULL), jobs_recovered(0), ai_ghost_recovered(false),
      uncreated_ids(), ai_mutex(NULL), ai_qs(), being_deleted(false)
...
{
  snprintf(this->ai_qs.parent_id, sizeof(this->ai_qs.parent_id), "%s",
           array_id);
}
...
{
  snprintf(this->ai_qs.fileprefix, sizeof(this->ai_qs.fileprefix), "%s",
           file_prefix);
}
...
{
  snprintf(this->ai_qs.submit_host, sizeof(this->ai_qs.submit_host), "%s",
           submit_host);
}
    ...
    if (strlen(pcnt) > 0) {
      ...
      if ((max_limit != NO_SLOT_LIMIT) &&
        ...
        snprintf(log_buf, sizeof(log_buf),
                 "Array %s requested a slot limit above the max limit %ld, "
                 "rejecting\n",
                 this->ai_qs.parent_id, max_limit);
        log_event(PBSEVENT_SYSTEM, PBS_EVENTCLASS_JOB, this->ai_qs.parent_id,
                  log_buf);
 ...
 *
 * @param requested_limit - the user-requested limit, or -1 if none was
 * requested
...

{
  ...
  if (get_svr_attr_l(SRV_ATR_IdleSlotLimit, &max_idle_slot_limit) ==
      PBSE_NONE) {
 ...
 * parse_array_request()
 *
 * Parses the array request, checks that we aren't above the maximum size, and
 * allocates the array for the job ids
 *
 * @param request - the user submitted array request
 * @return PBSE_NONE on success, or ARRAY_TOO_LARGE if we're above the maximum
 * size, or PBSE_MEM_MALLOC, or the rc from translate_range_string_to_vector if
 * an invalid range was submitted.
 */
...

{
  ...
  if ((rc = translate_range_string_to_vector(request, this->uncreated_ids)) !=
      PBSE_NONE)
    return (rc);
  ...
  this->ai_qs.array_size =
      this->uncreated_ids[this->uncreated_ids.size() - 1] + 1;
 ...
 *
 * @param internal_index - the index of uncreated_ids that corresponds to the
 * subjob index returned
 * @return the index of the next subjob to be created, or -1 if no job should be
 ...
 *
 * Creates a new array subjob if the array's idle count is below the idle slot
 * limit
 */
  ...
  if (next_index >= 0) {
    ...
    if (template_job != NULL) {
      ...
      if (hostname_extension != NULL)
        snprintf(prev_job_id, sizeof(prev_job_id), "%s[%d]%s", old_id,
                 this->ai_qs.highest_id_created, hostname_extension);
      else
        snprintf(prev_job_id, sizeof(prev_job_id), "%s[%d]", old_id,
                 this->ai_qs.highest_id_created);

      std::string prev_id(prev_job_id);

      int rc = create_and_queue_array_subjob(this, array_mgr, template_job,
                                             template_mgr, next_index, prev_id,
                                             false);
 ...
 * @param job_exit_status - the exit status, only used if the event is
 * aeTerminate
 */
...
void job_array::update_array_values(
    ...
    const char *job_id, int job_exit_status)

{
  ...
  case aeTerminate:
    ...
    if (get_svr_attr_b(SRV_ATR_MoabArrayCompatible, &moab_compatible) !=
        PBSE_NONE)
...
void job_array::mark_deleted() { this->being_deleted = true; }

bool job_array::is_deleted() const { return (this->being_deleted); }
