...
job::job()
    : ji_plugin_usage_info(), ji_momstat(0), ji_modified(0), ji_momhandle(-1),
      ji_radix(0), ji_has_delete_nanny(false), ji_qhdr(NULL), ji_lastdest(0),
 ...
 *
 * Adds resource usage information to the string for the accounting log from the
 * resource plugin, if any
 *
 * @param acct_data -
 */
...

{
  for (std::map<std::string, std::string>::const_iterator it =
           this->ji_plugin_usage_info.begin();
       it != this->ji_plugin_usage_info.end(); it++)
...

{
  for (std::map<std::string, std::string>::const_iterator it =
       ...
       it != this->ji_plugin_usage_info.end(); it++) {
    svrattrl *pal =
        attrlist_create(ATTR_used, it->first.c_str(), it->second.size() + 1);
