...
// This is set to true once we've begun exiting. At this point, global
// destructors have been called and we want to ignore requests
...
bool completed_jobs_map_class::add_job(
    ...
    const char *jobid, time_t complete_time)
...

{
  ...
  for (std::map<std::string, time_t>::iterator it = completed_jobs_map.begin();
    ...
    if (it->second <= now) {
      ...
      if ((pnew = (struct work_task *)calloc(1, sizeof(struct work_task))) ==
          NULL) {
        rc = -1;
        goto cleanup;
      }
      ...
      if ((pnew->wt_mutex =
               (pthread_mutex_t *)calloc(1, sizeof(pthread_mutex_t))) == NULL) {
        free(pnew->wt_parm1);
        free(pnew);
        rc = -1;
        goto cleanup;
      ...
      if (LOGLEVEL >= 10) {
        snprintf(log_buf, LOCAL_LOG_BUF_SIZE,
                 "%s: calling handle_complete_second_time()",
                 it->first.c_str());
      ...
      enqueue_threadpool_request((void *(*)(void *))handle_complete_second_time,
                                 pnew, task_pool);
    }
  }
  ...
  for (unsigned int i = 0; i < to_remove.size(); i++) {
    if (LOGLEVEL >= 10) {
      snprintf(log_buf, LOCAL_LOG_BUF_SIZE,
               "%s: removing job from completed_job_map", to_remove[i].c_str());
      log_event(PBSEVENT_DEBUG, PBS_EVENTCLASS_SERVER, __func__, log_buf);
    }
...

{
  ...
  if (completed_jobs_map.empty()) {
    ...
    for (std::map<std::string, time_t>::iterator it =
             completed_jobs_map.begin();
         it != completed_jobs_map.end(); ++it) {
