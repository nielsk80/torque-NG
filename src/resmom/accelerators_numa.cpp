...
#ifdef NVML_API
hwloc_obj_t Machine::get_non_nvml_device(hwloc_topology_t topology,
                                         nvmlDevice_t device) {
  hwloc_obj_t osdev;
  nvmlReturn_t nvres;
  nvmlPciInfo_t pci;
...
int Machine::initializeNVIDIADevices(hwloc_obj_t machine_obj,
                                     hwloc_topology_t topology) {
  nvmlReturn_t rc;

  /* Initialize the NVML handle.
   *
   * nvmlInit should be called once before invoking any other methods in the
   * NVML library. A reference count of the number of initializations is
  ...
  if (rc == NVML_SUCCESS) {
    ...
    for (unsigned int idx = 0; idx < device_count; idx++) {
      ...
      if (gpu_obj == NULL) {
        /* This was not an nvml device. We will look for a "card" device
         * (GeForce or Quadra) */
      ...
      ancestor_obj =
          hwloc_get_ancestor_obj_by_type(topology, HWLOC_OBJ_MACHINE, gpu_obj);
   ...
   *
   * nvmlShutdown should be called after NVML work is done, once for each call
   * to nvmlInit() A reference count of the number of initializations is
...
#ifdef MIC
int Chip::initializeMICDevices(hwloc_obj_t chip_obj,
                               hwloc_topology_t topology) {
  ...
  for (int idx = 0;; idx++) {
    ...
    if (chip_obj == NULL) { /* this came from the Non NUMA */
      PCI_Device new_device;
      ...
      PCI_Device new_device;
      ancestor_obj =
          hwloc_get_ancestor_obj_by_type(topology, HWLOC_OBJ_NODE, mic_obj);
      if (ancestor_obj != NULL) {
...
void PCI_Device::initializeMic(
    ...
    int idx, hwloc_topology_t topology)
...

{
  ...
  if (rc != NVML_SUCCESS) {
    ...
    if (nearest_cpuset != NULL) {
      ...
      hwloc_bitmap_list_snprintf(cpuset_string, MAX_CPUSET_SIZE,
                                 nearest_cpuset);
    }
  }
