...
void remote_notify_pmix_operation(job *pjob, const char *remote_host,
                                  const std::string &data, int op,
...
pmix_operation::pmix_operation(
    ...
    job *pjob, const pmix_proc_t procs[], size_t nprocs,
    const std::string &info, pmix_modex_cbfunc_t cbfunc, void *rdata)
    : data(info), hosts_reported(), hosts_to_report(), local_ranks(),
      all_ranks(), to_call_fence(cbfunc), to_call_connect(NULL), op_id(0),
      complete(false), type(FENCE_OPERATION), cbdata(rdata)
    ...
    for (size_t i = 0; i < nprocs; i++) {
      if (this->safe_insert_rank(pjob, procs[i].rank) == false) {
        ...
        snprintf(log_buffer, sizeof(log_buffer),
                 "Fence request from PMIx server requested rank %d but we only "
                 "have %d ranks",
...
/*
 * Constructor used for connect operations on the moms that are being notified
 * of the connect
 *
...
pmix_operation::pmix_operation(
    ...
    char *connect_data, job *pjob)
    : data(), hosts_reported(), hosts_to_report(), local_ranks(), all_ranks(),
  ...
  if (*ptr == ':') {
    do {
      ...
      if (this->safe_insert_rank(pjob, rank) == false) {
        snprintf(log_buffer, sizeof(log_buffer),
                 "Connect request from PMIx server via remote mom requested "
                 "rank %d but we only have %d ranks",
...
pmix_operation::pmix_operation(
    ...
    job *pjob, const pmix_proc_t procs[], size_t nprocs,
    pmix_op_cbfunc_t cbfunc, void *rdata)
    : data(), hosts_reported(), hosts_to_report(), local_ranks(), all_ranks(),
      to_call_fence(NULL), to_call_connect(cbfunc), op_id(0), complete(false),
      type(CONNECT_OPERATION), cbdata(rdata)

{
  for (size_t i = 0; i < nprocs; i++) {
    if (this->safe_insert_rank(pjob, procs[i].rank) == false) {
      ...
      snprintf(log_buffer, sizeof(log_buffer),
               "Connect request from PMIx server requested rank %d but we only "
               "have %d ranks",
               procs[i].rank, pjob->ji_numvnod);
      log_err(-1, pjob->ji_qs.ji_jobid, log_buffer);
    }
...
pmix_operation::pmix_operation(
    ...
    job *pjob, const pmix_proc_t procs[], size_t nprocs,
    pmix_op_cbfunc_t cbfunc, void *rdata, int op_type)
    : data(), hosts_reported(), hosts_to_report(), local_ranks(), all_ranks(),
      to_call_fence(NULL), to_call_connect(cbfunc), op_id(0), complete(false),
      type(op_type), cbdata(rdata)
...

{
  ...
  if (it != this->hosts_to_report.end()) {
    ...
    if (this->hosts_to_report.size() == 0)
      ...
      snprintf(log_buffer, sizeof(log_buffer),
               "Still waiting for %lu hosts to report for %s's fence "
               "operation. %s reported in.",
               this->hosts_to_report.size(), this->jobid.c_str(),
               hostname.c_str());
...
pmix_operation::pmix_operation()
    : jobid(), data(), hosts_reported(), hosts_to_report(), local_ranks(),
      all_ranks(), to_call_fence(NULL), to_call_connect(NULL), op_id(),
      complete(false), type(FENCE_OPERATION)
...
pmix_operation::pmix_operation(
    ...
    : jobid(other.jobid), data(other.data),
      hosts_reported(other.hosts_reported),
      hosts_to_report(other.hosts_to_report), local_ranks(other.local_ranks),
      all_ranks(other.all_ranks), to_call_fence(other.to_call_fence),
      to_call_connect(other.to_call_connect), op_id(other.op_id),
      complete(other.complete), type(other.type)
...

{
  ...
  for (std::set<int>::iterator it = this->all_ranks.begin();
       it != this->all_ranks.end(); it++) {
    if (ranks.size() != 0)
      sprintf(buf, ",%d", *it);
    else
 ...
 * complete_operation()
 *
 * Completes this objects pmix operation, which as of now is either fence or
 * connect For either, this includes notifying all participating moms and, for
 * me, executing the callback
 *
...

{
  ...
  for (std::set<std::string>::iterator it = this->hosts_reported.begin();
    ...
    if (*it == mom_alias) {
      ...
      if (this->type == FENCE_OPERATION)
        remote_notify_pmix_operation(pjob, it->c_str(), this->data, IM_FENCE,
                                     NULL);
      else if (this->type == CONNECT_OPERATION)
        remote_notify_pmix_operation(pjob, it->c_str(), ranks, IM_CONNECT,
                                     NULL);
      else {
        char buf[MAXLINE];
        sprintf(buf, "%d", this->op_id);
        remote_notify_pmix_operation(pjob, it->c_str(), buf, IM_DISCONNECT,
                                     NULL);
      }
    }
  }
...

{
  if (this->type == FENCE_OPERATION) {
    if (this->to_call_fence != NULL)
      this->to_call_fence(PMIX_SUCCESS, this->data.c_str(), this->data.size(),
                          this->cbdata, NULL, NULL);
...
unsigned int pmix_operation::get_operation_id() const { return (this->op_id); }
...
int pmix_operation::connection_process_terminated(
    ...
    job *pjob, struct sockaddr_in *source_addr, bool am_i_ms)

{
  int rc = PBSE_NONE;

  // Terminate all local tasks
  for (std::set<int>::iterator it = this->local_ranks.begin();
       it != this->local_ranks.end(); it++) {
    task *ptask = task_find(pjob, *it);

    if (ptask != NULL) {
      int task_rc = kill_task(pjob, ptask, SIGKILL, 0);
      if ((task_rc != PBSE_NONE) && (rc == PBSE_NONE))
        rc = task_rc;
    }
  }
  ...
  if (am_i_ms) {
    ...
    // Mother superior needs to tell all other members of this connection to
    // terminate
    for (std::set<std::string>::iterator it = this->hosts_reported.begin();
         it != this->hosts_reported.end(); it++) {
      if (*it == mom_alias)
      ...
      remote_notify_pmix_operation(pjob, it->c_str(), buf, IM_CONNECT,
                                   source_addr);
    }
  }
 ...
 * clean_up_connection()
 *
 * A process that is part of this connection has exited. Terminate the
 * connection
 *
...
int clean_up_connection(
    ...
    job *pjob, struct sockaddr_in *source_addr, unsigned int op_id,
    bool am_i_ms)

{
  int rc = PBSE_NONE;

  std::map<unsigned int, pmix_operation>::iterator it =
      existing_connections.find(op_id);

  if (it != existing_connections.end()) {
    rc = existing_connections[op_id].connection_process_terminated(
        pjob, source_addr, am_i_ms);
...

{
  ...
  for (std::map<unsigned int, pmix_operation>::iterator it =
           existing_connections.begin();
       it != existing_connections.end(); it++) {
  ...
  for (unsigned int i = 0; i < connection_ids.size(); i++) {
    clean_up_connection(pjob, NULL, connection_ids[i],
                        am_i_mother_superior(*pjob));
  }
