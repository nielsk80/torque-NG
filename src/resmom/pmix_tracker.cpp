...
int kill_job(job *pjob, int sig, const char *killer_id,
 ...
 * @return PBSE_NONE on SUCCESS, -1 if the task wasn't found, or a return from
 * kill_task
 */
...
int pmix_tracker::abort(
    ...
    int status, pmix_proc_t proc_list[], size_t nprocs, const char abort_msg[])
...
{
  bool matches =
      strncmp(jobint_string, this->jid.c_str(), strlen(jobint_string)) == 0;
