...
public:
  ...
  void set_job_state(
      torque_ng::Job::JobState state,
      torque_ng::Job::JobSubState sub_state = torque_ng::Job::SUBSTATE_NONE);
  void set_interactive(bool interactive);

  // Resource Management
  void set_resources_used(uint64_t mem, uint64_t vmem, double walltime);
  void set_resource_limits(uint64_t mem, uint64_t vmem, uint32_t nodes,
