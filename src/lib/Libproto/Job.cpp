...
Job::Job(std::string_view job_id) : m_msg(std::make_unique<JobStatusUpdate>()) {
    m_msg->set_job_id(std::string(job_id));
}

void Job::set_owner(std::string_view owner) {
    m_msg->set_user_owner(std::string(owner));
}

void Job::set_state(int32_t state, int32_t sub_state) {
    m_msg->set_job_state(state);
    m_msg->set_sub_state(sub_state);
}

void Job::set_interactive(bool interactive) {
    m_msg->set_is_interactive(interactive);
}

void Job::set_resources_used(uint64_t mem, uint64_t vmem, double walltime) {
...
void Job::set_resource_limits(uint64_t mem, uint64_t vmem, uint32_t nodes, uint32_t procs, double walltime) {
...
void Job::set_execution_info(std::string_view host, int32_t exit_status) {
    m_msg->set_exec_host(std::string(host));
    m_msg->set_exit_status(exit_status);
}

void Job::set_timestamps(uint64_t qtime, uint64_t mtime) {
    m_msg->set_queue_time(qtime);
    m_msg->set_modify_time(mtime);
}

void Job::add_attribute(std::string_view key, std::string_view value) {
    (*m_msg->mutable_attributes())[std::string(key)] = std::string(value);
}

void Job::add_env_var(std::string_view key, std::string_view value) {
    (*m_msg->mutable_environment_vars())[std::string(key)] = std::string(value);
}

std::string_view Job::get_id() const {
    return m_msg->job_id();
}
...
std::unique_ptr<Job> Job::deserialize(const std::string& data) {
