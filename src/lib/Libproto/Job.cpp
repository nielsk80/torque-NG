...
JobEntity::JobEntity(std::string_view job_id)
    : m_msg(std::make_unique<torque_ng::JobStatusUpdate>()) {
...
void JobEntity::set_job_state(torque_ng::Job::JobState state,
                              torque_ng::Job::JobSubState sub_state) {
...
void JobEntity::set_resources_used(uint64_t mem, uint64_t vmem,
                                   double walltime) {
...
void JobEntity::set_resource_limits(uint64_t mem, uint64_t vmem, uint32_t nodes,
                                    uint32_t procs, double walltime) {
...
std::string_view JobEntity::get_id() const { return m_msg->job_id(); }

std::string JobEntity::serialize() const { return m_msg->SerializeAsString(); }
