...
attr_req_info &attr_req_info::operator=(const attr_req_info &newattr_req_info) {
  ...
  default_swap_value.atsv_shift =
      newattr_req_info.default_swap_value.atsv_shift;
  default_disk_value.atsv_num = newattr_req_info.default_disk_value.atsv_num;
  default_disk_value.atsv_shift =
      newattr_req_info.default_disk_value.atsv_shift;

  return *this;
}

attr_req_info::attr_req_info()
    : max_lprocs(0), max_mem(0), max_swap(0), max_disk(0), max_nodes(0),
...
int attr_req_info::set_min_limit_value(
    ...
    const char *rescn, const char *val)
...
unsigned int attr_req_info::get_unsigned_min_limit_value(
    ...
    const char *rescn, unsigned long &value)
...
int attr_req_info::get_max_values(std::vector<std::string> &names,
...
int attr_req_info::get_min_values(std::vector<std::string> &names,
                                  std::vector<std::string> &values) {
  char buf[100];
...
int attr_req_info::get_default_values(std::vector<std::string> &names,
                                      std::vector<std::string> &values) {
  char buf[100];
  ...
  for (unsigned int i = 0; i < names.size(); i++) {
    ...
    if (names[i].find("lprocs") == 0)
      ...
      if (values[i] == use_cores) {
        if ((this->max_cores > 0) && (this->max_cores < lprocs))
          return (PBSE_EXLIMIT);
      }

      if ((this->max_threads > 0) && (this->max_threads < lprocs))
        return (PBSE_EXLIMIT);
...

{
  ...
  for (unsigned int i = 0; i < names.size(); i++) {
    ...
    if (names[i].find("lprocs") == 0)
      ...
      if (values[i] == use_cores) {
        if ((this->min_cores > 0) && (this->min_cores > lprocs))
          return (PBSE_MINLIMIT);
      } else if (this->min_cores > 0)
        return (PBSE_MINLIMIT);

      if ((this->min_threads > 0) && (this->min_threads > lprocs))
        return (PBSE_MINLIMIT);
 ...
 *
 * @param names  -  vector of strings with resource names that are part of a
 request
 * @param values -  vector of strings which are the values for values in the
 names parameter
 * @param names_to_add - a vector of strings containing the names of default
 values which were not in the user's request.
 * @param values_to_add - a vector of strings containing the values of default
 values which were not set in the user's request.
 */
    ...
    for (unsigned int j = 0; j < names.size(); j++) {
      ...
      if (!strcmp(dflt_name.c_str(), req_name.c_str())) {
        if (!strcmp(dflt_name.c_str(), LPROCS)) {
          /* lprocs is a special case. We always get at least one lprocs. If set
             to one assume user did not request lprocs */
