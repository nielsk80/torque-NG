...
void complete_req::set_value_from_nodes(
    ...
    const char *node_val, int &task_count)
 ...
 * @param legacy_vmem - true if vmem should be considered per node, false if per
 * job
 */
...

{
  ...
  if ((legacy_vmem == false) && (task_count != 0))
    mem /= task_count;
...
int complete_req::set_task_memory_used(int req_index, int task_index,
...
int complete_req::set_task_cput_used(int req_index, int task_index,
 ...
 * @return PBSE_NONE if the name is valid and the index is >= 0,
...
int complete_req::set_value(
    ...
    int index, const char *name, const char *value, bool is_default)
...
int complete_req::get_req_index_for_host(
    ...
    const char *host, unsigned int &req_index)
...
int complete_req::get_req_and_task_index(
    ...
    const int rank, unsigned int &req_index, unsigned int &task_index)

{
  int tasks_counted = 0;

  for (unsigned int req_count = 0; req_count < this->req_count(); req_count++) {
    for (unsigned int task_count = 0;
         task_count < this->reqs[req_count].getTaskCount(); task_count++) {
      int rc;
      allocation al;
      ...
      for (unsigned int cpus_per_task = 0;
           cpus_per_task < al.cpu_indices.size(); cpus_per_task++) {
 ...
 * @param local_rank (I) - the rank for this process among processes on this
 * node
 ...
 * @return PBSE_NONE - if we could locate a local rank for this job,
 * PBSE_NO_PROCESS_RANK otherwise. Returning PBSE_NO_PROCESS_RANK should cause
 * this to be placed in the cgroup for the entire host
 */

int complete_req::get_req_and_task_index_from_local_rank(

    int local_rank, unsigned int &req_index, unsigned int &task_index,
...
int complete_req::update_hostlist(
    ...
    const std::string &host_spec, int req_index)

{
  if ((req_index < 0) || (req_index >= this->reqs.size())) {
    return (-1);
  }
...
void complete_req::set_hostlists(
    ...
    const char *job_id, const char *host_list)

{
  if ((job_id == NULL) || (host_list == NULL))
    return;
  ...
  while (current != NULL) {
    ...
    if (i >= this->reqs.size()) {
      ...
      snprintf(log_buf, sizeof(log_buf),
               "We received %d or more req assignments for job %s which has "
               "only %d reqs.",
...
int complete_req::get_task_stats(
    ...
    unsigned int &req_index, std::vector<int> &task_index,
...
void complete_req::set_task_usage_stats(
    ...
    int req_index, int task_index, unsigned long cput_used,
...
unsigned int complete_req::get_num_reqs() { return (this->reqs.size()); }
