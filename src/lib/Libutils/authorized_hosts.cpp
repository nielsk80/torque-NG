 ...
 */
bool authorized_hosts::is_authorized_impl(unsigned long addr,
...
void authorized_hosts::add_authorized_address(unsigned long addr,
                                              unsigned short port,
  ...
  std::string msg = "Added authorized host: " + host + " (" + format_ip(addr) +
                    ":" + std::to_string(port) + ")";
  log_event(Torque::EventType::Debug, Torque::EventClass::Server, "Auth",
            msg.c_str());
}
