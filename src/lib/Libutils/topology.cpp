...
// This is going to replace numa_socket.cpp, numa_chip.cpp, numa_core.cpp, and
// numa_thread.cpp. It will be used to determine the topology of the system and
// how many sockets, chips, cores, and threads are present.
...
void HardwareNode::display(int depth) const {
  ...
  std::string msg = indent + "[Type " + std::to_string(static_cast<int>(type)) +
                    "] " + "ID: " + std::to_string(id) +
                    " | Threads: " + std::to_string(available_threads) + "/" +
...
std::string HardwareNode::get_cgroup_path() const {
  ...
  return path + "/" + type_to_string(this->type) + "_" +
