    ...
    if (pbs_conf.is_open()) {
      ...
      while (std::getline(pbs_conf, line)) {
        ...
        if (line.compare(0, 9, "PBS_HOME=") == 0) {
          ...
          pbs_home.erase(std::remove(pbs_home.begin(), pbs_home.end(), '\"'),
                         pbs_home.end());
...
Machine::Machine() {
  ...
  log_event(Torque::EventType::Debug, Torque::EventClass::Server, "Machine",
            msg.c_str());
}
...
bool Machine::has_sufficient_resources(int cores_requested,
...
bool Machine::schedule_job(const Job &job) {
  ...
  if (pid < 0) {
    log_err(static_cast<int>(Torque::ErrorCode::SystemError), __func__,
            "Failed to fork for job" + std::to_string(job.id));
    return false;
  }
  ...
  if (pid == 0) {
    ...
    if (!user.apply_to_current_process()) {
      log_err(static_cast<int>(Torque::ErrorCode::Internal), __func__,
              "Child failed: Unable to drop privileges" + job.user);
      _exit(1);
    }
    ...
    std::string msg = "Job " + std::to_string(job.id) +
                      " successfully launched and isolated.";
    log_event(Torque::EventType::System, Torque::EventClass::Server, "Machine",
              msg.c_str());
