...
NodeType TopologyFactory::map_hwloc_type(hwloc_obj_type_t type) {
  ...
  case HWLOC_OBJ_OS_DEVICE:
    return NodeType::Accelerator;
  default:
    return NodeType::Machine;
  }
}

void TopologyFactory::walk_topology(HardwareNode *parent, hwloc_obj_t hw_obj,
  ...
  auto node =
      std::make_unique<HardwareNode>(our_type, hw_obj->logical_index, parent);

  // --- Capture Memory (Specifically from NUMA nodes) ---
  if (our_type == NodeType::NumaNode &&
      hw_obj->attr->numanode.local_memory > 0) {
  ...
  if (our_type == NodeType::Core && hw_obj->cpuset) {
    ...
    if (kind_idx >= 0) {
      int efficiency = 0;
      if (hwloc_cpukinds_get_info(topo, kind_idx, nullptr, &efficiency, nullptr,
                                  nullptr, 0) == 0) {
...
std::unique_ptr<HardwareNode> TopologyFactory::build_system_tree() {
  ...
  auto root_node =
      std::make_unique<HardwareNode>(root_type, root_obj->logical_index);
