...
public:
  ...
  virtual void setup_job_cgroup(int job_id,
                                const std::string &aggregate_cpuset) = 0;
  virtual void migrate_pid(int job_id, pid_t pid) = 0;
  virtual void cleanup_job_cgroup(int job_id) = 0;
};

// The No-Op implementation for environments where cgroups are not available or
// desired
