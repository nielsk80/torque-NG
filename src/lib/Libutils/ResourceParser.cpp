...
uint64_t ResourceParser::string_to_size(std::string_view size_str) {
  ...
  auto first_alpha =
      std::find_if(size_str.begin(), size_str.end(),
  ...
  std::string_view suffix =
      size_str.substr(std::distance(size_str.begin(), first_alpha));
  ...
  if (iequals(suffix, "kb") || iequals(suffix, "k"))
    return value * 1024ULL;
  if (iequals(suffix, "mb") || iequals(suffix, "m"))
    return value * 1024ULL * 1024ULL;
  if (iequals(suffix, "gb") || iequals(suffix, "g"))
    return value * 1024ULL * 1024ULL * 1024ULL;
  if (iequals(suffix, "tb") || iequals(suffix, "t"))
    return value * 1024ULL * 1024ULL * 1024ULL * 1024ULL;
  if (iequals(suffix, "pb") || iequals(suffix, "p"))
    return value * 1024ULL * 1024ULL * 1024ULL * 1024ULL * 1024ULL;
...
/**
 * @brief Main entry point to parse a full resource string (e.g.,
 * "mem=2gb,nodes=1:ppn=2").
...
void ResourceParser::parse_into(std::string_view raw_resources,
  ...
  if (raw_resources.empty() || target == nullptr)
    return;
 ...
 * Now supports legacy Torque colon-delimited sub-resources (e.g.,
 * nodes=1:ppn=8).
 */
