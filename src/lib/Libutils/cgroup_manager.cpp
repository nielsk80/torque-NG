...
public:
  void setup_job_cgroup(int job_id,
                        const std::string &aggregate_cpuset) override {
    ...
    try {
      if (!fs::exists(base_path)) {
        ...
        std::ofstream(base_path / "cgroup.subtree_control")
            << "+cpuset +memory +cpu";
      }
      ...
      std::string msg =
          "Created isolation container for job " + std::to_string(job_id);
      log_event(Torque::EventType::System, Torque::EventClass::Server, "TORQUE",
                msg);
    } catch (const std::exception &e) {
      log_err(static_cast<int>(Torque::ErrorCode::Internal), __func__,
              e.what());
    }
  }
  ...
  void migrate_pid(int job_id, pid_t pid) override {
    ...
    std::string path =
        "/sys/fs/cgroup/torque/job_" + std::to_string(job_id) + "/cgroup.procs";
    ...
    try {
      ...
      if (!outfile.is_open()) {
        ...
        log_err(
            static_cast<int>(Torque::ErrorCode::Internal), routine.c_str(),
            ("Failed to open cgroup.procs for job " + std::to_string(job_id))
                .c_str());
        return;
      }
      outfile << pid;
    } catch (const std::exception &e) {
      log_err(static_cast<int>(Torque::ErrorCode::Internal), routine.c_str(),
              e.what());
    }
  }
  ...
  void cleanup_job_cgroup(int job_id) override {
    ...
    namespace fs = std::filesystem;
    fs::path job_path =
        fs::path("/sys/fs/cgroup/torque") / ("job_" + std::to_string(job_id));
...
std::unique_ptr<ICgroupManager>
CgroupManagerFactory::create(const std::string &type) {
  if (type == "v1") {
    log_event(Torque::EventType::Debug, Torque::EventClass::Server, "CgroupMgr",
              "Creating V1 Manager");
