...
void UserContext::lookup_user() {
  ...
  if (getpwnam_r(name.c_str(), &pwd, buffer.data(), buffer.size(), &result) ==
          0 &&
      result != nullptr) {
    uid = pwd.pw_uid;
    gid = pwd.pw_gid;
    ...
    log_event(Torque::EventType::Error, Torque::EventClass::Server, "Security",
              "User lookup failed: " + name);
  }
}
...
bool UserContext::apply_to_current_process() const {
  ...
  // The following three steps must be done in order to properly drop
  // privileges:
  ...
  if (getuid() != uid || geteuid() != uid) {
    log_event(Torque::EventType::Error, Torque::EventClass::Server, "Security",
              "ID verification failed after setuid");
    return false;
  }
