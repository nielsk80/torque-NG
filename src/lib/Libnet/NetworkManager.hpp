 ...
 * * This class replaces the legacy C-based networking in net_server.c and net_client.c.
 * It implements a length-prefixed framing protocol to ensure Protobuf messages
 * are received in their entirety over a TCP stream.
 */
class NetworkManager {
public:
     ...
     * @return true if all bytes were read, false if the connection closed/errored.
     */
