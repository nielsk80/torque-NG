...
public:
  // We pre-allocate space for a maximum number of nodes to avoid frequent
  // remapping
...
private:
  ...
  // Note: Strings (names) are handled differently since they aren't
  // fixed-width. For this version, we store names in a standard vector and only
  // mmap the numeric data.
