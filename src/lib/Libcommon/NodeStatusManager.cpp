...
NodeStatusManager::NodeStatusManager(const std::string& state_file, size_t max_nodes)
    : max_nodes_(max_nodes),
      // Calculate total size: (State + CPU + Mem) * max_nodes
      buffer_(state_file, max_nodes * (sizeof(Node::NodeState) + sizeof(int32_t) + sizeof(uint64_t))) {
    ...
    pcpus_ptr_   = reinterpret_cast<int32_t*>(raw_base + (max_nodes_ * sizeof(Node::NodeState)));
    mem_ptr_     = reinterpret_cast<uint64_t*>(raw_base + (max_nodes_ * (sizeof(Node::NodeState) + sizeof(int32_t))));
}
...
void NodeStatusManager::update_node(const Node& node_update) {
    ...
    // Direct memory writes - These are immediately "visible" to the OS for persistence
