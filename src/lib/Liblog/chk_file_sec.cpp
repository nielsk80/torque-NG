...
bool IamRoot() { return (geteuid() == 0); }

int validate_path_security(const fs::path &path, bool is_dir, int disallow_mask,
                           bool sticky_check) {
  std::error_code ec;
  fs::path target = path;
  ...
  if (!exists) {
    ...
    if (target.empty())
      target = "."; // Default to current dir if no path provided
  ...
  if (st.st_uid != 0 && st.st_uid != geteuid()) {
    std::cerr << "torque-ng [SECURITY]: " << target
              << " owned by unauthorized UID " << st.st_uid << std::endl;
    return EPERM;
  }
  ...
  if ((current_mode & disallow_mask) != 0) {
    std::cerr << "torque-ng [SECURITY]: " << target
              << " has insecure bits: " << std::oct
              << (current_mode & disallow_mask) << std::dec << std::endl;
    return EACCES;
  }
