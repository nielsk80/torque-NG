 ...
 */
void log_record(int eventtype, int objclass, std::string_view objname,
 ...
 */
std::error_code log_open(std::string_view filename,
 ...
 */
long log_size() { return SafeLog::getInstance().get_size_kb(); }

/**
 * log_close - Cleanly closes the singleton's stream.
 */
void log_close(int msg) { SafeLog::getInstance().close(); }
