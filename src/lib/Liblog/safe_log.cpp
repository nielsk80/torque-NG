...
void SafeLog::record(Torque::EventType type, Torque::EventClass cls, 
    ...
    if (!(static_cast<uint32_t>(type) & m_event_mask)) return;

    // 3. Direct Write: Since we hold m_mutex, we can use the stream safely
    if (m_logstream.is_open()) {
        m_logstream << "[" << get_timestamp() << "] "
                    << "Host: " << m_hostname << " | "
...
bool SafeLog::open(const std::filesystem::path& path) {
    ...
    if (validate_path_security(path, true, S_IWOTH) != 0) {
        std::cerr << "Security Failure: Log path " << path << " is insecure." << std::endl;
        return false;
    }
...
void log_set_hostname_sharelogging(std::string_view server_name, std::string_view server_port) {
    ...
    if (status == 0 && res != nullptr) {
        ...
        std::string error_msg = "Critical Networking Alert: Unable to resolve local hostname '";
        ...
        SafeLog::getInstance().record(Torque::EventType::Security, Torque::EventClass::Server, "Network", error_msg);
 ...
 */
bool SafeLog::rotate() {
    // Note: We assume the caller (record) or a scoped_lock here handles concurrency
    if (m_logpath.empty() || !m_logstream.is_open()) return false;
    ...
    if (ec) {
        std::cerr << "torque-ng [ERROR]: Failed to rotate log: " << ec.message() << std::endl;
    }
